...
import cloudscraperRequest from './utils/cloudscraper.js'
import format from 'string-format'
import { writeFile } from 'fs'
// import Xray from 'x-ray-scraper/Xray'
import makeDriver from './utils/makeDriver.js'
import * as filters from './utils/filters.js'
import { isString, isArray, uniqueName, oneArgument, twoArguments } from './utils/helpers.js'
...
export default class TorrentProvider {
  ...
  enableProvider(...args) {
    if (!this.requireAuthentification) {
      this.isActive = true;
    } else if (this.supportCredentialsAuthentification && TorrentProvider.isCredentialsAuthentification(args)) {
  ...
  getCategoryValue(categoryName) {
    ...
    const categoryKey = Object.keys(this.categories).find(key => uniqueName(key) === uniqueName(categoryName));
  ...
  downloadTorrent(torrent, path) {
    return this.downloadTorrentBuffer(torrent).then(buffer =>
      path ? writeFileAsync(path, buffer) : buffer
    )
  }
